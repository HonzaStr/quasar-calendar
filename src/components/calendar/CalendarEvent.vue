<template>
  <div :class="getEventClass()" :style="getEventStyle()" @click="handleClick">
    <template
      v-if="renderStyle === 'singleLine' || isAllDayEvent() || (renderStyle === 'doubleLine' && eventDuration() < 45)"
    >
      <template
        v-if="!eventHasPreviousDay() || ((firstDayOfWeek || isLeftmostColumn) && eventHasPreviousDay())"
      >
        <div v-if="isEmptySlot()" class="calendar-event-summary">&nbsp;</div>
        <div v-else class="calendar-event-summary calendar-event-render-single">
          <span
            v-if="!isAllDayEvent()"
            class="calendar-event-time"
          >{{ formatTime(eventObject.start.dateObject) }}</span>
          {{ eventObject.summary }}
        </div>
      </template>
    ...
    <template v-else-if="renderStyle === 'doubleLine'">
      <div class="calendar-event-summary calendar-event-render-single">{{ eventObject.summary }}</div>
      <div
        class="calendar-event-time calendar-event-render-single"
      >{{ formatTimeRange(eventObject.start.dateObject, eventObject.end.dateObject) }}</div>
    </template>
    <template v-else>
      ...
      <template v-if="!eventHasPreviousDay() || (firstDayOfWeek && eventHasPreviousDay())">
        <span
          v-if="!isAllDayEvent() && showTime"
          class="calendar-event-start-time"
        >{{ formatTime(eventObject.start.dateObject) }}</span>
        <span v-if="isEmptySlot()" class="calendar-event-summary">&nbsp;</span>
        <span v-else class="calendar-event-summary">{{ eventObject.summary }}</span>
      </template>
      <template v-else>&nbsp;</template>
    </template>
  </div>
...
import { CalendarMixin, CalendarEventMixin, EventPropsMixin } from './mixins'
...
export default {
  ...
  methods: {
    ...
    isLastDayOfMonth: function(dateObject) {
      ...
      return (
        this.makeDT(this.currentCalendarDay).toISODate() ===
        this.makeDT(dateObject)
          .endOf('month')
          .toISODate()
      )
    },
