<template>
  <div class="calendar-agenda column fit">
    ...
    >
      <div v-for="daysForward in localNumDays" :key="daysForward">
        <div v-if="forwardDate = getDaysForwardDate(daysForward - 1)">
          ...
          >{{ formatDate(forwardDate, 'MMMM yyyy') }}</div>
          ...
          >
            ...
            >
              <div class="calendar-agenda-side-date">{{ formatDate(forwardDate, 'd') }}</div>
              <div class="calendar-agenda-side-day">{{ formatDate(forwardDate, 'EEE') }}</div>
            </div>
    ...
    <div v-else>
      ...
      <div v-for="daysForward in numJumpDays" :key="daysForward">
        <div
        ...
        >
          ...
          >
            ...
            >
              <div class="calendar-agenda-side-day">{{ formatDate(forwardDate, 'EEE') }}</div>
              <div class="calendar-agenda-side-date">{{ formatDate(forwardDate, 'MMM d') }}</div>
            </div>
            <div class="col row calendar-agenda-events">
              <div
                class="full-width"
...
import { QBtn, QTooltip, QScrollArea, QInfiniteScroll, QSpinnerDots } from 'quasar'
export default {
  name: 'CalendarAgenda',
  components: {
    CalendarAgendaEvent,
  ...
  methods: {
    ...
    doUpdate: function() {
      this.mountSetDate()
      this.triggerDisplayChange(this.eventRef, this.getAgendaDisplayDates())
    },
