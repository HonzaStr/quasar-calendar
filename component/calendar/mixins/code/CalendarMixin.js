...
export default {
  ...
  methods: {
    ...
    triggerEventClick: function(eventObject, eventRef) {
      this.$root.$emit('click-event-' + eventRef, eventObject)
    },
    triggerDayClick: function(dateObject, eventRef) {
      this.$root.$emit('click-day-' + eventRef, {
    ...
    triggerDisplayChange: function(eventRef, payload) {
      ...
      this.$root.$emit('display-change-' + eventRef, payload)
    },
    ...
    fullMoveToDay: function(dateObject) {
      if (this.fullComponentRef) {
        this.$root.$emit(this.fullComponentRef + ':moveToSingleDay', {
    ...
    getForcedWeekDateArray: function(numberOfDays, sundayFirstDayOfWeek) {
      ...
      for (let counter = 0; counter <= numberOfDays - 1; counter++) {
        returnArray.push(this.makeDT(bookendDates.first).plus({ days: counter }))
      }
      return returnArray
    },
    getWeekDateArray: function(numberOfDays) {
      let returnArray = []
      for (let counter = 0; counter <= numberOfDays - 1; counter++) {
        returnArray.push(this.makeDT(this.workingDate).plus({ days: counter }))
      }
      return returnArray
    },
    formatTimeFromNumber: function(hourNumber, minuteNumber = 0) {
      ...
      return localeFormattedHour.replace(' ', '').toLowerCase()
    },
    ...
    handleDateChange: function(params) {
      ...
      this.triggerDisplayChange(this.eventRef, {
    ...
    isCurrentDate: function(thisDateObject) {
      return DateTime.local().hasSame(this.makeDT(thisDateObject), 'day')
    },
    ...
    calculateDayCellWidth: function(numberOfDays) {
      return this.decimalAdjust('floor', 100 / numberOfDays, -3) + '%'
    },
    ...
    getDayHourId: function(eventRef, workingDate, thisHour) {
      return eventRef + '-' + this.makeDT(workingDate).toISODate() + '-hour-' + thisHour
    },
  },
  mounted() {},
}
